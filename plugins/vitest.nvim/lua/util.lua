...
function M.search_ancestor(startpath, func)
...
function M.search_ancestors(startpath, func)
	...
	local guard = 100
	local dirs = {}
	-- todo: define end path as this should be .git
	for path in M.path.iterate_parents(startpath) do
		-- Prevent infinite recursion if our algorithm breaks
		guard = guard - 1
		if guard == 0 then
...
function M.root_pattern(...)
	...
	return function(startpath)
		return M.search_ancestor(startpath, matcher)
	end
end

function M.find_git_ancestor(startpath)
	return M.search_ancestor(startpath, function(path)
...
--- @param startpath string: The path to start searching from.
--- @returns table: The path to the nearest package.json file.
function M.find_node_modules_ancestors(startpath)
