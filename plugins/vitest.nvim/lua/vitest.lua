...
local function hasVitestDependency(path)
	...
	if gitRootPath and rootPath ~= gitRootPath then
		local monorepoSuccess, monorepoRootPackageJsonContent = pcall(lib.files.read, gitRootPath .. "/package.json")
...
local function getVitestCommand(path)
	...
	for _, nodeModulesPath in ipairs(nodeModulesPaths) do
		local binary = util.path.join(nodeModulesPath, "node_modules", ".bin", "vitest")
		if util.path.exists(binary) then
			return binary
		end
	end

	-- navigate up to git ancestor to find vitest
